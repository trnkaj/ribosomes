...
a <- read.csv("./figure5de+supplfigs5de/exp19_20_22_min_max.csv")
...
plot(m1_min_max, depth = 2)
precis(m1_min_max, depth = 2)
#trankplot(ribo_m6_green_sirna, depth = 2)

post_m1_mm <- extract.samples(m1_min_max)

#destandardise samples
post_m1_mm$a_bar_norm <- post_m1_mm$a_bar * sd(a$log10_alexa) + mean(a$log10_alexa)
#post_m1_mm$a <- post_m1_mm$a * sd(a$log10_alexa) + mean(a$log10_alexa)
post_m1_mm$b_treat_norm <- post_m1_mm$b_treat * sd(a$log10_alexa)
post_m1_mm$b_mix_control_norm <- post_m1_mm$b_mix_control * sd(a$log10_alexa)
post_m1_mm$b_mix_sirna_norm <- post_m1_mm$b_mix_sirna * sd(a$log10_alexa)

#predict values for measurements with treat = 0, mix_control = 0, mix_sirna = 0, i.e. control
mu_control <- 10^(post_m1_mm$a_bar_norm + post_m1_mm$b_treat_norm*0 + post_m1_mm$b_mix_control_norm*0 + post_m1_mm$b_mix_sirna_norm*0)
mu_sirna <- 10^(post_m1_mm$a_bar_norm + post_m1_mm$b_treat_norm*1 + post_m1_mm$b_mix_control_norm*0 + post_m1_mm$b_mix_sirna_norm*0)
mu_control_mix <- 10^(post_m1_mm$a_bar_norm + post_m1_mm$b_treat_norm*0 + post_m1_mm$b_mix_control_norm*1 + post_m1_mm$b_mix_sirna_norm*0)
mu_sirna_mix <-10^(post_m1_mm$a_bar_norm + post_m1_mm$b_treat_norm*1 + post_m1_mm$b_mix_control_norm*0 + post_m1_mm$b_mix_sirna_norm*1)

#plot density of the posterior distributions for control, sirna, control_mix and sirna_mix
#save to pdf
pdf("./figure5f+supplfigs5f/bayes_min_max_posterior.pdf", width = 8, height = 6)
...
#save to pdf
pdf("./figure5f+supplfigs5f/bayes_min_max_posterior_difference.pdf", width = 8, height = 6)
