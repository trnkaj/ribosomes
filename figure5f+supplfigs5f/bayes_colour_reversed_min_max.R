...
a <- read.csv("./figure5de+supplfigs5de/colour-reversed-exp23_27_28_min_max.csv")
...
#####varying intercepts
m1_reverse_colour_min_max <- ulam(
  alist(
...
plot(m1_reverse_colour_min_max, depth = 2)
precis(m1_reverse_colour_min_max, depth = 2)
#trankplot(ribo_m6_green_sirna, depth = 2)

post_m1_rcmm <- extract.samples(m1_reverse_colour_min_max)

#destandardise samples
post_m1_rcmm$a_bar_norm <- post_m1_rcmm$a_bar * sd(a$log10_alexa) + mean(a$log10_alexa)
#post_m1_rcmm$a <- post_m1_rcmm$a * sd(a$log10_alexa) + mean(a$log10_alexa)
post_m1_rcmm$b_treat_norm <- post_m1_rcmm$b_treat * sd(a$log10_alexa)
post_m1_rcmm$b_mix_control_norm <- post_m1_rcmm$b_mix_control * sd(a$log10_alexa)
post_m1_rcmm$b_mix_sirna_norm <- post_m1_rcmm$b_mix_sirna * sd(a$log10_alexa)

#predict values for measurements with treat = 0, mix_control = 0, mix_sirna = 0, i.e. control
mu_control <- 10^(post_m1_rcmm$a_bar_norm + post_m1_rcmm$b_treat_norm*0 + post_m1_rcmm$b_mix_control_norm*0 + post_m1_rcmm$b_mix_sirna_norm*0)
mu_sirna <- 10^(post_m1_rcmm$a_bar_norm + post_m1_rcmm$b_treat_norm*1 + post_m1_rcmm$b_mix_control_norm*0 + post_m1_rcmm$b_mix_sirna_norm*0)
mu_control_mix <- 10^(post_m1_rcmm$a_bar_norm + post_m1_rcmm$b_treat_norm*0 + post_m1_rcmm$b_mix_control_norm*1 + post_m1_rcmm$b_mix_sirna_norm*0)
mu_sirna_mix <-10^(post_m1_rcmm$a_bar_norm + post_m1_rcmm$b_treat_norm*1 + post_m1_rcmm$b_mix_control_norm*0 + post_m1_rcmm$b_mix_sirna_norm*1)

#plot density of the posterior distributions for control, sirna, control_mix and sirna_mix
#save to pdf
pdf("./figure5f+supplfigs5f/bayes_colour_reversed_min_max_posterior.pdf", width = 8, height = 6)
...
#save to pdf
pdf("./figure5f+supplfigs5f/bayes_colour_reversed_min_max_posterior_difference.pdf", width = 8, height = 6)
