...
LoadFc <- function(path){
  file <- read.FCS(path, transformation = FALSE, truncate_max_range = FALSE)
  exprs <- exprs(file)
  df <- as.data.frame(exprs)
  return(df)
}
...
single_cells_pos_exp19 <- sicon_sicon_exp19 %>% filter (`FSC-A` > 100000 & `FSC-A` <260000 & `FSC-H` > 50000 & `FSC-H` < 150000 & `FITC-H` > 0)

# FITC minimum threshold based on FITC negative population (siRNA treated cells)
...
fitc_min_exp19 <- round(10^(mean(log10(single_cells_neg_exp19$`FITC-H`)) + 2 * sd(log10(single_cells_neg_exp19$`FITC-H`))))

# FITC maximum threshold based on FITC positive population (CFSE stained cells)
...
fitc_max_exp19 <- round(10^(mean(log10(single_cells_pos_exp19$`FITC-H`)) - 2 * sd(log10(single_cells_pos_exp19$`FITC-H`))))

# Use FITC gates as variables for mixed sample
fitc_mix_con_exp19 <- fitc_max_exp19
fitc_mix_sirna_exp19 <- fitc_min_exp19

# FITC gates visualisation
gates_plot_19 <- bind_rows(
  neg = single_cells_neg_exp19,
  pos = single_cells_pos_exp19,
  .id = "gate_type")

ggplot(gates_plot_19, aes(x = `FSC-H`, y = `FITC-H`, colour = gate_type))+
  geom_point(size = 1, alpha = 0.3)+
  geom_hline(yintercept = fitc_max_exp19)+
  geom_hline(yintercept = fitc_min_exp19)+
  scale_y_log10()+
  theme_classic()
...
single_cells_pos_exp20 <- sicon_sicon_exp20 %>% filter (`FSC-A` > 100000 & `FSC-A` <260000 & `FSC-H` > 50000 & `FSC-H` < 150000 & `FITC-H` > 0)

# FITC minimum threshold based on FITC negative population (siRNA treated cells)
# FITC minimum threshold gate is defined as mean (negative FICT-H) plus two times standard deviation (strict gate to exclude party stained cells)
fitc_min_exp20 <- round(10^(mean(log10(single_cells_neg_exp20$`FITC-H`)) + 2 * sd(log10(single_cells_neg_exp20$`FITC-H`))))

# FITC maximum threshold based on FITC positive population (CFSE stained cells)
# FITC maximum threshold gate is defined as mean (positive FITC-H) minus two times standard deviation (strict gate to exclude party stained cells)
fitc_max_exp20 <- round(10^(mean(log10(single_cells_pos_exp20$`FITC-H`)) - 2 * sd(log10(single_cells_pos_exp20$`FITC-H`))))

# Use FITC gates as variables for mixed sample
fitc_mix_con_exp20 <- fitc_max_exp20
fitc_mix_sirna_exp20 <- fitc_min_exp20

# FITC gates visualisation
gates_plot_20 <- bind_rows(
  neg = single_cells_neg_exp20,
  pos = single_cells_pos_exp20,
  .id = "gate_type")

ggplot(gates_plot_20, aes(x = `FSC-H`, y = `FITC-H`, colour = gate_type))+
  geom_point(size = 1, alpha = 0.3)+
  geom_hline(yintercept = fitc_max_exp20)+
  geom_hline(yintercept = fitc_min_exp20)+
  scale_y_log10()+
  theme_classic()
...
single_cells_pos_exp22 <- sicon_sicon_exp22 %>% filter (`FSC-A` > 100000 & `FSC-A` <260000 & `FSC-H` > 50000 & `FSC-H` < 150000 & `FITC-H` > 0)

# FITC minimum threshold based on FITC negative population (siRNA treated cells)
# FITC minimum threshold gate is defined as mean (negative FICT-H) plus two times standard deviation (strict gate to exclude party stained cells)
fitc_min_exp22 <- round(10^(mean(log10(single_cells_neg_exp22$`FITC-H`)) + 2 * sd(log10(single_cells_neg_exp22$`FITC-H`))))

# FITC maximum threshold based on FITC positive population (CFSE stained cells)
# FITC maximum threshold gate is defined as mean (positive FITC-H) minus two times standard deviation (strict gate to exclude party stained cells)
fitc_max_exp22 <- round(10^(mean(log10(single_cells_pos_exp22$`FITC-H`)) - 2 * sd(log10(single_cells_pos_exp22$`FITC-H`))))

# Use FITC gates as variables for mixed sample
fitc_mix_con_exp22 <- fitc_max_exp22
fitc_mix_sirna_exp22 <- fitc_min_exp22

# FITC gates visualisation
gates_plot_22 <- bind_rows(
  neg = single_cells_neg_exp22,
  pos = single_cells_pos_exp22,
  .id = "gate_type")

ggplot(gates_plot_22, aes(x = `FSC-H`, y = `FITC-H`, colour = gate_type))+
  geom_point(size = 1, alpha = 0.3)+
  geom_hline(yintercept = fitc_max_exp22)+
  geom_hline(yintercept = fitc_min_exp22)+
  scale_y_log10()+
  theme_classic()
...
write.csv(a, "./figure5de+supplfigs5de/exp19_20_22_min_max.csv", row.names = FALSE)
